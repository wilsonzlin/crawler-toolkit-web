...
// TODO We'd like to match by full eTLD, but a giant regex is extremely slow (just check the CPU profiler). As a reasonable approximation, we only match on the TLD, which can be done by simply splitting on "." and looking at the last component.
export const TLDS = new Set(
  PUBLIC_SUFFIX_LIST_RAW.split("\n")
    .map((l) => l.trim())
...
export const resolveUrl = (
  ...
  return normaliseUrl(raw.toString(), allowQuery);
};

// Why use this over `splitString` or `.split`?
