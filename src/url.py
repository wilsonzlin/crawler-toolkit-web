...
// ===END PRIVATE DOMAINS===
"""
PUBLIC_SUFFIX_LIST: List[str] = []
for l in PUBLIC_SUFFIX_LIST_RAW.split("\n"):
    l = l.strip()
    if l != "" and not l.startswith("//"):
        l = re.sub(r"^\*\.", "", l)
        PUBLIC_SUFFIX_LIST.append(l)


# If this is not True, it means iterating by characters will not be Unicode code points (i.e. not UTF-32).
assert sys.maxunicode == 0x10FFFF


def valid_sqs_char(c: int) -> bool:
    # It looks like AWS is disallowing:
    # - ASCII control characters except tab, LF, and CR.
    # - All Unicode code points in the BMP except for the UTF-16 surrogates, 0xFFFE, and 0xFFFF (the last two most likely because they could be used as BOM).
    # - All Unicode code points in all supplementary planes.
    return (
        c == 0x9
        or c == 0xA
        or c == 0xD
...
def normalise_url(raw: str, allow_query: bool = False) -> Optional[str]:
    for c in raw:
        c = ord(c)
        if not valid_sqs_char(c):
            return None
    # We accept a URL prefixed with the `https://` protocol, or directly starts with the origin.
    # We do not accept:
    # - Non-HTTPS protocols (e.g. data URIs, JavaScript).
    # - Usernames and/or passwords (`@`).
    # - Ports (`:`).
    # - Hostnames that contain characters outside of [a-z0-9-].
    # - Hostnames without a TLD.
    # - Missing path.
    prefix = re.match(r"^(?:https://)?((?:[a-z0-9-]+\.)+(?:[a-z0-9-]+))/", raw)
    if not prefix:
        return None
    origin = prefix.group(1)
    if not any(origin.endswith("." + etld) for etld in PUBLIC_SUFFIX_LIST):
        return None
    path_raw = raw[prefix.end() :]
    # Use `re.search` as `re.match` anchors to the start of the string.
    m = re.search(r"[?#]", path_raw)
    query = ""
    if not m:
        pathname_raw = path_raw
    else:
        pathname_raw = path_raw[: m.start()]
        if allow_query and m.group(0) == "?":
            # Include the question mark.
            query, _, _ = path_raw[m.start() :].partition("#")
            # It's not valid to normalise the query string (i.e. sort by param name):
            # - Sometimes the order is significant e.g. `?list[]=5&list[]=2`.
            # - Sometimes the query string is used arbitrarily e.g. `?{"json":"object"}`.
            # - TODO Should we canonicalise percent encoding?
    has_trailing_slash = len(pathname_raw) > 0 and pathname_raw[-1] == "/"
    pathname = "/".join(
        (
            unquote_plus(c)
            .replace("%", "%25")
            ...
            .replace("#", "%23")
            .replace("/", "%2F")
            for c in pathname_raw.split("/")
            if c != ""
        )
    )
    if pathname == "":
        # Avoid double slashes if we append trailing slash.
        pathname = "/"
    else:
        pathname = "/" + pathname + ("/" if has_trailing_slash else "")
    norm = f"{origin}{pathname}{query}"
    # Set an upper limit on URLs, as otherwise this could cause issues with index entry size limits in PostgreSQL.
    # Note that it's weird for a URL to be very long, unless `allow_query` is enabled. Query strings can get crazy; possibilities include access tokens, image configuration, tracking, IDs, base64 encoding, etc.
    if len(norm.encode("utf-8")) >= 2048:
        return None
    return norm


def resolve_url(base: str, href: str, allow_query: bool = False) -> Optional[str]:
    if href:
        try:
            # We don't need to use urlsplit and try and manually assert parts of the URL. However, if we do:
            # - Use urlsplit over urlparse as the latter will split off ';' in the pathname.
            # - Accessing `port` will throw an exception if it's not a valid port number.
            # - Do not manually build URL string when passing to `normalise_url` as `p.query` does NOT include the question mark.
            return normalise_url(urljoin(f"https://{base}", href), allow_query)
        except:
            ...
    return None
